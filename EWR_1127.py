...
from utils.MAL import MAL,defocusfitting,normalize
#选择数据
from data.stackimage320 import *
...
def train():
    ...
    # images2 = np.pad(image,((int(padsize/2),int(padsize/2)),(int(padsize/2),int(padsize/2)),(0,0)),'constant',constant_values = 0 )
    ...
    [x.qxa,x.qya] = np.array(np.meshgrid(x.qx,x.qy, indexing='ij'))
    stackSize = [x.highth + padsize, x.width + padsize, x.page]
    if ifaligned == False:
        x.dxy = np.zeros((stackSize[2], 2))
        x.dxy[:,0] = align_x
        x.dxy[:,1] = align_y

    for a0 in range(0, stackSize[2]):
        if ifaligned == False:
            psiShift =  np.exp(-2j*np.pi*x.pixelsize*(x.qxa*x.dxy[a0,0] + x.qya*x.dxy[a0,1]))
    ...
    if imagetype == 'simulated':
        images1 = images2
    defocusrecord = np.zeros([len(defocus_series),21])
    ...
    for i in trange(N_iters):
        ...
        #         pixelsize,img1,E0 = E0,Cs = Cs,alpha = alpha,M = 5,deltaC1 = deltaC1)
        
        #     if (i + 1) % 2000 == 0:
        #         # print("orignal defocus",defocus_series[k])
        #         # print("fit defocus",defoucs)
        #         print("fit defocus",defocus_series)
        #     defocus_series[k] = defoucs
        ...
        #更新神经网络中的权重      
        if i <= N_iters *2/3 and ifdefoucsfitting == True:
